...
Player::Player(const std::string &file_name) :
	...
	format_converter_{ std::make_unique<FormatConverter>(video_decoder_->width(), video_decoder_->height(),video_decoder_->pixel_format(), AV_PIX_FMT_YUV420P) },
	...
	try
	{
		...
		for (;;)
		{
			...
			while (!sent)
			{
				...
				{
					frame_decoded->pts = av_rescale_q(frame_decoded->pkt_dts,demuxer_->time_base(),microseconds);
					...
					if (av_image_alloc(frame_converted->data,
